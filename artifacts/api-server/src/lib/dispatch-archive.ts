...
export async function listDispatchArchive(limit = 50): Promise<DispatchArchiveSummary[]> {
  const rows = await db
    .select({
      ...
      evalRubricVersion: dispatchArchiveTable.evalRubricVersion,
      evalJudgeModels: dispatchArchiveTable.evalJudgeModels,
      evalPairwise: dispatchArchiveTable.evalPairwise,
      evalUnsubs24h: dispatchArchiveTable.evalUnsubs24h,
      evalUnsubRate24h: dispatchArchiveTable.evalUnsubRate24h,
      evalEngagementRunAt: dispatchArchiveTable.evalEngagementRunAt,
